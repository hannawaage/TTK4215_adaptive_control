function dx = problem1_d(t, x, m, beta, g, l, A, B, C_phi_1, C_phi_2, C_phi_3, C_z, D_z, Gamma ,K)
% True system states
% x(1) = q
% x(2) = q_dot
% Filter states
...
phi_1 = C_phi_1*x(3:4); % phi_1 = -s/Lambda(s) q
phi_2 = C_phi_2*x(5:6); % phi_2 = -1/Lambda(s) sin(q)
phi_3 = C_phi_3*x(7:8); % phi_3 = 1/Lambda(s) tau
phi = [phi_1; phi_2; phi_3];
theta = x(11:13);
epsilon = (z-theta'*phi)/(1+phi'*phi);
u = -K*[x(1)-pi;x(2)];
dx_1 = x(2);
dx_2 = u/(m*l^2)-beta/(m*l^2)*x(2)-(g/l)*sin(x(1));
dx_34 = A*x(3:4) + B*q;
